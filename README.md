...
```
git clone https://github.com/rafaelcmmarques/CRUD-Livros.git
```

2. Abra o projeto no NetBeans (`File > Open Project`) apontando para a pasta `CRUD-Livros` clonada.

3. Certifique-se de que o `db4o.jar` está referenciado nas bibliotecas do projeto (`Properties > Libraries`).

4. Compile e execute diretamente pelo NetBeans (`Run > Run Project` ou `F6`), ou via terminal com Ant:

```
ant run
```

5. Alternativamente, gere o `.jar` e execute-o manualmente:

```
ant jar
java -jar dist/CRUD-Livros.jar
```
